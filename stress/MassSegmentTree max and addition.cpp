    ...
    while (true) {
        ...
        for (int j = 0; j < q; ++j) {
            ...
            if (t == 0) {
                ...
                if (l > r) swap(l, r);
                ++r;
                int m1 = segtree.get(l, r);
                int m2 = 0;
                for (int i = l; i < r; ++i) m2 = max(m2, data[i]);
                assert(m1 == m2);
            } else if (t == 2) {
                int l = rng() % n, m = rng() % M;
                int r1 = segtree.lastTrue(l, [&](int mx, int r) { return mx <= m; });
                int m2 = 0, r2 = l;
                while (r2 < n && max(m2, data[r2]) <= m) {
                    m2 = max(m2, data[r2]);
                    r2++;
                }
                assert(r1 == r2 || ((r1 == segtree.n) && (r2 == n)));
            } else if (t == 3) {
                int r = rng() % n, m = rng() % (n * M);
                int l1 = segtree.firstTrue(r, [&](int mx, int r) { return mx <= m; });
                int m2 = 0, l2 = r;
                while (l2 > 0 && max(m2, data[l2 - 1]) <= m) {
                    l2--;
                    m2 = max(m2, data[l2]);
                }
                assert(l1 == l2);
